...
class Preferences:
    ...
    def _set_initial_values(self):
        self.ufw_logging.set_active(UFW_LEVEL2NUM[self.gufw.fw.get_ufw_logging()])
        if self.gufw.fw.get_logging():
            self.gufw_logging.set_active(True)
        if self.gufw.fw.get_config_value('ConfirmDeteleRule') == 'yes':
            self.gufw_confirm_delete.set_active(True)
        
        if self.gufw.fw.get_config_value('RefreshInterval'):
            self.report_interval.set_value(int(self.gufw.fw.get_config_value('RefreshInterval')))
        else:
            self.report_interval.set_value(3)
        
        # Profiles (model)
        self.profile_rows = Gtk.ListStore(str)
        for profile in self.gufw.fw.get_all_profiles():
    ...
    def _rename_profile(self, widget, path, new_name):
        ...
        if previous_name == self.gufw.fw.get_profile():
            self.gufw.show_dialog(self.win_preferences, _("Current profile"), _("You can't rename the current profile"))
            return
        
        self.gufw.fw.rename_profile(self.profile_rows[path][0], new_name)
    ...
    def on_AddProfile_btn_clicked(self, widget, data=None):
        ...
        self.gufw.fw.add_profile(new_name)
        ind = self.gufw.fw.get_all_profiles().index(new_name)
    ...
    def on_DeleteProfile_btn_clicked(self, widget, data=None):
        ...
        if treeiter == None:
            ...
            if deleted_profile == self.gufw.fw.get_profile():
                self.gufw.show_dialog(self.win_preferences, _("Profile not erasable"), _("You can't remove the current profile"))
            else:
                self.gufw.profile.remove(self.gufw.fw.get_all_profiles().index(deleted_profile))
                self.gufw.fw.delete_profile(deleted_profile)
    ...
    def on_ufw_logging_changed(self, widget, data=None):
        self.gufw.fw.set_ufw_logging(NUM2UFW_LEVEL[self.ufw_logging.get_active()])
    ...
    def on_report_interval_scale_button_release_event(self, widget, data=None):
        ...
        self.gufw.fw.set_config_value('RefreshInterval', refresh_time)
